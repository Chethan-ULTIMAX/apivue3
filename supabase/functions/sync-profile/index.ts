...
Deno.serve(async (req) => {
  ...
  if (!adapter) {
    return json({ error: `Unsupported platform "${platform}". Supported: ${supportedPlatforms.join(", ")}` }, 400);
  }

  let profile;
  try {
    profile = await adapter.fetchProfile(handle);
  } catch (err) {
    return json({ error: (err as Error).message || "Could not fetch this profile" }, 502);
  }
