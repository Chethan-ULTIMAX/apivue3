...
Deno.serve(async (req) => {
  ...
  const { data: existing } = await supabase.from("tracked_profiles").select("data").eq("user_id", userId).eq("platform", platform).eq("handle", handle).maybeSingle();
  const previous = (existing?.data ?? {}) as Record<string, unknown>;
  const preservedVerification = previous.ownershipVerified === true
    ? { ownershipVerified: true, verificationMethod: previous.verificationMethod, verificationVerifiedAt: previous.verificationVerifiedAt }
    : {};
  const mergedProfile = { ...profile, ...preservedVerification };
  ...
  const { data: saved, error: upsertError } = await supabase.from("tracked_profiles").upsert({
    ...
    profile_url: mergedProfile.profileUrl,
    data: mergedProfile,
    sync_error: null,
  ...
  for (const m of mergedProfile.metrics) if (typeof m.value === "number") metrics[m.key] = m.value;
  const totalActivity = mergedProfile.activity.reduce((s, a) => s + a.count, 0);
