...
export function TrendAnalytics({ hasData, report }: TrendAnalyticsProps) {
  ...
  const activity = useMemo(() => {
    const points = report.trends
      .flatMap((trend) => trend.points)
      .reduce<Map<string, number>>((map, point) => {
    ...
    <section>
      <Card className="overflow-hidden border-border bg-card/60">
          ...
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Historical changes across your collected activity.
              </p>
            </div>
        ...
        <CardContent className="p-5">
          {hasData && report.trends.length > 0 ? (
            <TrendReadyState activity={activity} topTrend={topTrend} />
          ) : (
            <TrendEmptyState />
          )}
        </CardContent>
...
function TrendEmptyState() {
  return (
    <div className="relative flex min-h-[330px] items-center justify-center overflow-hidden rounded-xl border border-dashed border-border bg-muted/30 p-6">
      ...
      <div className="relative max-w-md text-center">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl border border-violet-500/20 bg-violet-500/10">
          <LineChartIcon className="h-5 w-5 text-violet-500 dark:text-violet-300" />
        </div>
        <h3 className="mt-4 text-sm font-semibold">Your trend line will appear here</h3>
        <p className="mt-2 text-xs leading-5 text-muted-foreground">
          APIVue needs at least two numeric snapshots for a genuine historical trend.
        </p>
        <div className="mt-5 flex flex-wrap justify-center gap-2">
          <Pill icon={CalendarRange} text="Daily" />
          <Pill icon={Activity} text="Weekly" />
          <Pill icon={BarChart3} text="Monthly" />
        </div>
      </div>
    </div>
  );
...
function TrendReadyState({
  ...
  topTrend: ProgressReport['trends'][number];
}) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-muted/30 p-4">
        <div>
          <p className="text-xs uppercase tracking-wider text-muted-foreground">Strongest tracked change</p>
          <p className="mt-1 text-sm font-semibold">{topTrend.label} · @{topTrend.handle}</p>
        </div>
        <div className="text-right">
          <p className={`text-lg font-bold tabular-nums ${topTrend.change >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-destructive'}`}>
            {topTrend.change >= 0 ? '+' : ''}{topTrend.change.toLocaleString()}
          </p>
          <p className="text-[11px] text-muted-foreground">
            {topTrend.changePct === null ? 'absolute change' : `${topTrend.changePct}% change`}
          </p>
        </div>
      </div>
      ...
      <div className="h-[260px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Line type="monotone" dataKey="count" stroke="hsl(var(--primary))" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        Calculated from stored activity history — no synthetic points.
      </div>
    </div>
  );
}
