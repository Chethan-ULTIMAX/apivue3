...
export async function connectPublicProfile(platform: 'leetcode' | 'codewars', handle: string): Promise<SyncProfile> {
  const clean = handle.trim().replace(/^@/, '');
  if (!clean) throw new Error('A username or platform handle is required.');
  const provider = platform === 'leetcode' ? 'LeetCode' : 'Codewars';
  const confirmed = window.confirm(`Is this your correct ${provider} username?\n\n@${clean}\n\nAPIVue will add this public profile to your account. No ownership verification is required.`);
...
async function startOwnershipVerification(platform: 'leetcode' | 'codewars', handle: string): Promise<OwnershipChallenge> { const clean = handle.trim().replace(/^@/, ''); if (!clean) throw new Error('A username is required.'); const { data, error } = await supabase.functions.invoke('start-profile-verification', { body: { platform, handle: clean } }); if (error) throw new Error(error.message); const result = data as { error?: string; challengeId?: string } & Partial<OwnershipChallenge>; if (result.error) throw new Error(result.error); if (!result.code || !result.expiresAt || !result.instructions) throw new Error('The verification challenge was incomplete.'); const challenge = { ...result, challengeId: result.challengeId } as OwnershipChallenge; sessionStorage.setItem('apivue.ownership.challenge', JSON.stringify(challenge)); return challenge; }
export async function beginOwnershipVerification(platform: 'leetcode' | 'codewars', handle: string): Promise<void> { await startOwnershipVerification(platform, handle); window.location.assign(`${import.meta.env.BASE_URL}dashboard/integrations/verify/${platform}`); }
export function getOwnershipChallenge(): OwnershipChallenge | null { try { const raw = sessionStorage.getItem('apivue.ownership.challenge'); return raw ? JSON.parse(raw) as OwnershipChallenge : null; } catch { return null; } }
export function clearOwnershipChallenge(): void { sessionStorage.removeItem('apivue.ownership.challenge'); }
export async function verifyOwnership(platform: 'leetcode' | 'codewars', handle: string, code: string): Promise<void> { if (platform === 'codewars') { const challenge = getOwnershipChallenge(); if (!challenge?.challengeId) throw new Error('The Codewars verification challenge is missing. Start verification again.'); const { data, error } = await supabase.functions.invoke('check-profile-ownership', { body: { platform, handle, challengeId: challenge.challengeId } }); if (error) throw new Error(error.message); const result = data as { error?: string; verified?: boolean; expired?: boolean }; if (result.error) throw new Error(result.error); if (result.expired) throw new Error('The Codewars verification challenge expired. Generate a new one.'); if (!result.verified) throw new Error('Codewars has not confirmed the webhook yet. Save the webhook in Codewars and try again.'); clearOwnershipChallenge(); return; } const { data, error } = await supabase.functions.invoke('verify-profile-ownership', { body: { platform, handle, code } }); if (error) throw new Error(error.message); const result = data as { error?: string; verified?: boolean }; if (result.error) throw new Error(result.error); if (!result.verified) throw new Error('Ownership verification did not complete.'); clearOwnershipChallenge(); }
...
export async function completeStackOverflowOAuth(code: string, state: string): Promise<void> { const verifier = sessionStorage.getItem(STACKOVERFLOW_PKCE_KEY); if (!verifier) throw new Error('Stack Overflow sign-in session is missing. Please start the connection again.'); const form = new URLSearchParams({ client_id: STACKOVERFLOW_CLIENT_ID, redirect_uri: STACKOVERFLOW_REDIRECT_URI, code, code_verifier: verifier }); const response = await fetch('https://stackoverflow.com/oauth/access_token/json', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', Accept: 'application/json' }, body: form.toString() }); const text = await response.text(); let payload: { access_token?: string; error?: { type?: string; message?: string }; error_message?: string } = {}; try { payload = text ? JSON.parse(text) : {}; } catch { /* handled below */ } if (!response.ok || !payload.access_token) { const providerMessage = payload.error?.message ?? payload.error_message; throw new Error(providerMessage ? `Stack Overflow OAuth failed: ${providerMessage}` : `Stack Overflow OAuth token exchange failed (${response.status}).`); } const { data, error } = await supabase.functions.invoke('stackoverflow-oauth-complete', { body: { state, access_token: payload.access_token } }); sessionStorage.removeItem(STACKOVERFLOW_PKCE_KEY); if (error) throw new Error(error.message); const result = data as { error?: string }; if (result.error) throw new Error(result.error); }

export async function getIntegrationStatus(): Promise<IntegrationStatus> { if (API_BASE) { try { return await backendRequest<IntegrationStatus>('/api/integrations'); } catch { /* serverless fallback below */ } } const { data, error } = await supabase.from('tracked_profiles').select('platform,handle,display_name,avatar_url,profile_url,last_synced_at,data').order('last_synced_at', { ascending: false }); if (error) throw new Error(error.message); const status: IntegrationStatus = { github: { connected: false }, codeforces: { connected: false }, leetcode: { connected: false }, codewars: { connected: false }, stackoverflow: { connected: false } }; for (const row of data ?? []) { const id = row.platform as IntegrationId; if (!(id in status) || status[id].connected) continue; const payload = row.data as Record<string, unknown> | null; const privateAccess = payload?.privateAccess === true || payload?.private_access === true; const repoCount = typeof payload?.accessibleRepoCount === 'number' ? payload.accessibleRepoCount : typeof payload?.accessible_repo_count === 'number' ? payload.accessible_repo_count : undefined; const ownershipVerified = payload?.ownershipVerified === true || payload?.ownership_verified === true; status[id] = { connected: true, username: row.handle, handle: row.handle, displayName: row.display_name, avatarUrl: row.avatar_url, profileUrl: row.profile_url ?? undefined, lastSyncedAt: row.last_synced_at ?? undefined, privateAccess, accessibleRepoCount: repoCount, ownershipVerified, verificationMethod: typeof payload?.verificationMethod === 'string' ? payload.verificationMethod : undefined }; } return status; }
...
if (typeof window !== 'undefined') { const params = new URLSearchParams(window.location.search); if (params.get('stackoverflow') === 'oauth_callback') { const code = params.get('code'); const state = params.get('state'); if (code && state) { void completeStackOverflowOAuth(code, state).then(() => { const url = new URL(window.location.href); url.searchParams.delete('code'); url.searchParams.delete('state'); url.searchParams.set('stackoverflow', 'connected'); window.location.replace(url.toString()); }).catch((error) => { const url = new URL(window.location.href); url.searchParams.delete('code'); url.searchParams.delete('state'); url.searchParams.set('stackoverflow', 'error'); url.searchParams.set('message', error instanceof Error ? error.message : 'Stack Overflow connection failed.'); window.location.replace(url.toString()); }); } } }
