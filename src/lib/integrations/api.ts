...
async function invokeOAuth(functionName: string, provider: string, body: Record<string, unknown> = {}): Promise<void> {
  ...
  if (error) {
    let message = error.message;
    const context = (error as { context?: { json?: () => Promise<unknown> } }).context;
    if (context?.json) { try { const responseBody = await context.json() as { error?: string }; if (responseBody?.error) message = responseBody.error; } catch { /* keep default */ } }
    throw new Error(message);
  }
...
export async function completeStackOverflowOAuth(code: string, state: string): Promise<void> {
  ...
  if (!response.ok || !payload.access_token) {
    const providerMessage = payload.error?.message ?? payload.error_message;
    throw new Error(providerMessage ? `Stack Overflow OAuth failed: ${providerMessage}` : `Stack Overflow OAuth token exchange failed (${response.status}).`);
  }
...
export async function connectStackOverflow(_handle?: string): Promise<void> {
  const verifier = randomVerifier();
  const challenge = await createPkceChallenge(verifier);
  sessionStorage.setItem(STACKOVERFLOW_PKCE_KEY, verifier);
  await invokeOAuth('stackoverflow-oauth-init', 'Stack Overflow', { code_challenge: challenge });
}
